...
export function init(): void {
	...
	task.spawn(() => {
		...
		const register = (folder: Instance): void => {
			const model = folder.WaitForChild(BUTTON_MODEL, 10);
			const part = model?.WaitForChild(BUTTON_PART, 10);
			const prompt = part?.WaitForChild(PROMPT_NAME, 10);
			if (prompt?.IsA("ProximityPrompt")) {
				prompts.set(folder.Name, prompt);
			} else {
